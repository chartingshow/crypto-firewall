...
(async function() {
    ...
    const cryptos = [...new Set([...top100Cryptos, 'BTC', 'ETH', 'XRP', 'LTC', 'ADA', 'DOGE', 'DOT', 'LINK', 'BNB', 'SOL', 'SUI', 'APT', 'SEI', 'ARB', 'OP', 'S', 'NEAR', 'TON'])];
    
    const priceRegex = new RegExp(`(\\$[\\d,]+(\\.\\d{2})?)\\s*(${cryptos.join('|')})`, 'gi');
    ...
    function highlightPrices() {
        ...
        while (node = walker.nextNode()) {
            ...
            if (matches) {
                ...
                span.innerHTML = node.nodeValue.replace(priceRegex, (match, price, cents, symbol) => {
                    const color = getColorForPrice(parseFloat(price.replace('$', '').replace(',', '')));
    ...
    function createPriceSummary() {
        ...
        summary.style.cssText = 'position: fixed; top: 10px; right: 10px; background-color: rgba(255,255,255,0.9); padding: 10px; border-radius: 5px; box-shadow: 0 0 10px rgba(0,0,0,0.1); z-index: 9999; max-height: 80vh; overflow-y: auto;';
        summary.innerHTML = '<h3>Cryptocurrency Prices</h3><ul id="crypto-price-list"></ul>';
