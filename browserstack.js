...
const {server, port, start, stop} = require('./tests/server')
...
const formatTestResults = (testResults) => {
  const formattedTestEnds = testResults
    .map((testResult) => {
...
const printResults = (results) => {
  ...
  const passedModules = moduleEnds.filter(
    (moduleEnd) => moduleEnd.status === 'passed',
  )
  const notPassedModules = moduleEnds.filter(
    (moduleEnd) => moduleEnd.status !== 'passed',
  )

  const testEnds = results
    .map(({testEnds}) => {
      return testEnds
    })
    .flat()
  const passedTests = testEnds.filter((testEnd) => testEnd.status === 'passed')
  const notPassedTests = testEnds.filter(
    (testEnd) => testEnd.status !== 'passed',
  )
  ...
  console.log(
    `Test modules:\t${formatTestsString(
      passedModules,
      notPassedModules,
      moduleEnds,
    )}`,
  ...
  console.log(
    `Tests:\t\t${formatTestsString(passedTests, notPassedTests, testEnds)}`,
  )
