...
const DEFAULT_THREATS = [
  ...
  { package: 'pdf-to-office', versions: '^1.1.2', reason: 'Atomic/Exodus wallet clipper' },
  { package: 'ethers-provider2', versions: '*', reason: 'Reverse shell installer' },
...
async function fetchLatestThreatList() {
  try {
    ...
    console.warn('⚠️  Could not fetch updated threat list, using cached version');
    return null;
  }
}
...
async function updateThreatListIfNeeded() {
  try {
    ...
    if (shouldUpdate) {
      ...
      if (freshThreats) {
        fs.writeFileSync(THREAT_LIST_PATH, JSON.stringify(freshThreats, null, 2));
...
function isMalicious(pkgName, pkgVersion) {
  ...
  return threats.some(threat => {
    return threat.package === pkgName && 
           satisfies(pkgVersion, threat.versions);
  });
}
...
(async () => {
  try {
    ...
    if (!scanPassed) {
      ...
      console.error('Recommendation: Run "npm audit" and review the packages mentioned above');
      process.exit(1);
    }
